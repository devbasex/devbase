...
| 書き込みの結果が分からない（送った後の接続断・タイムアウト・本文の途中切れ・5xx） | `SecretUnreachableError` | **消す** | — |
| 書き込みをサーバが拒んだ（403 以外の 4xx、解釈できない応答） | `SecretUnreachableError` | 残す | — |
...
だけが失われることがあり、控えを残すと `env delete` で消えた機密が次の不達でコンテナへ
戻る。サーバが拒んだと確定した応答（403 と、それ以外の 4xx）ではサーバは変わっておらず、
読んだときに進んだ世代がそのまま写しとして正しい。
