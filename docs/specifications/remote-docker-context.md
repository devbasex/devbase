...
ネストを付けず `settings.context` だけを残す。明示と devbase の解決結果は ssh の有無に
よらず付けるが、`docker context show` の推測は ssh 先のときに限るため、推測だけが元の
ときは空文字で `settings.context` も消える。その場合は恒久化の案内を出さない。
