...
`devbase list` の TUI の「環境変数」の操作は「キーの一覧と編集」「認証情報の再同期 (sync)」「初期セットアップ (init)」
...
3. キーの一覧が出る。各行はキー・持ち主（チーム / 個人）・適用範囲（共通 / プロジェクト）・グループ・値で、
   値は常に `******` で伏せる。同じキーがチームと個人の両方にあれば両方の行が出る。プロジェクトを選んだときは
   そのプロジェクトのキーだけが出て、共通のキーは出ない
...
| 操作 | 内容 |
|---|---|
| 先頭の「＋ キーを追加」 | キー名 → 持ち主 → 値の順に入れる。持ち主は個人単位の機密を持つ backend（OpenBao）だけ選ぶ。書き込み先の範囲は開いた範囲（共通か、選んだプロジェクト）になる |
| 行を選んで「値を変更」 | 同じ置き場の値を入れ直す |
| 行を選んで「削除」 | 確認で「はい」を選んだときだけ消す |

- 値は伏せ字の欄で入れ、画面に出しません。空の値・改行を含む値・前後に空白を含む値は受け付けません（改行や前後の
  空白を含む値は `devbase env edit` で編集してください）
