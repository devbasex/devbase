...
除外設定の判定は書き方の揺れを許容します。ルート指定（`/.env`）、任意階層（`**/.env`）、末尾スラッシュの有無（`secrets` / `secrets/`）、行末コメントや前後の空白は同じものとして扱われるため、正しく除外できていれば「不足」と報告されません。逆に `!secrets/` のような再包含や、`secrets/*.age` のように配下の一部だけを除外する書き方は不足として報告します。
