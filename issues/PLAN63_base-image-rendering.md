...
- [ ] 13. 追加するのは 6 パッケージの指定だけで、依存を含めて新規に入るのは 24 パッケージ、
       **イメージの増分が 7.09GB に対して +0.5% 未満（40 MB 以下）**に収まる
       検証: `devbase build base --no-cache` の前後の `docker images` の差
       **合否のラインを「約 24 MB」にしない。** 24 MB は稼働中のコンテナでの `du` の差で、
       apt のリストとキャッシュを含む測り方である。`docker images` が出すのは層単位の値で、
       層の重なりの分だけずれる。同じ数字を 2 つの測り方で比べると、正しい実装でも
       不合格になりうる
