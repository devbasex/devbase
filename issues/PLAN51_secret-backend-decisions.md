...
### 決定 14: `--user` は持ち主の軸だけを切り替える

`-p` は適用範囲の軸を、`--user` は持ち主の軸を選び、片方の指定がもう片方の軸を動かさない
（[設計 2 の入出力の契約](PLAN51_secret-backend-contract.md)）。書き込む `set` / `delete` /
`edit` は指定の無い軸に既定（適用範囲は共通、持ち主はチーム単位）を採り、読む `list` / `get` は
指定の無い軸を絞らない。

**現行の形をそのまま持ち主の軸へ写す。** `-p` は名前を取らない真偽フラグで、対象のプロジェクトは
実行時のディレクトリから決まる（`cli.py:293,301,310,314`）。読む側の既定は共通だけではなく、
`list` は共通と現在のプロジェクトの両方を出し（`env.py:345`）、`get` は共通に無ければ
プロジェクトを探す（`env.py:410`）。適用範囲の軸が読みと書きで既定を違えているこの形へ持ち主の
軸を重ねると、`--user` を付けない実行の結果が現行のまま残る（前提 3）。

| 案 | 採らない理由 |
| --- | --- |
| `--user` が適用範囲も共通へ固定する | `--user -p` で個人のプロジェクトを指せなくなり、4 象限のうち 1 つが CLI から届かない |
| `-p <name>` のように名前を取る形へ変える | 引数を取らない前提で書かれた既存の `devbase env set KEY=VALUE -p` が失敗する。実行位置からプロジェクトを決める `_target_env()` の判断も二重になる |
| 書き込みの既定も指定の無い軸へ広げる | 書き込みは宛先が 1 つに決まらないと成り立たない。既定を個人単位へ寄せる案は前提 3 に反する |
| `list` / `get` の既定をチーム単位だけへ絞る | `devbase up` は 4 層すべてを重ねるため、既定の一覧と取得がコンテナへ載る値を映さなくなる |

`env init` / `sync` / `project` と `export` / `import` へは `--user` を足さない。前者は `-p` を
持たず扱う参照が役割と実行位置で決まっており、後者はチーム単位の参照だけを相手にする
（決定 13）。個人単位へ値を入れる経路は `devbase env set --user` に一本化する。
...
| `--user` が持ち主の軸だけを切り替える | 偽サーバに対する結合テスト（`--user -p` の書き込みが個人のプロジェクトの `secretPath` へ届き、`-p` だけの書き込みは現行どおりチームのプロジェクトへ届くこと） |
| 既定の `list` / `get` の範囲が現行と同じ | `tests/commands/test_env_ops.py` と `tests/commands/test_env_store_switch.py` を書き換えずに通すこと（`age` / `plaintext` / 未設定で、`list` の節の構成と `get` の探索結果が変わらないこと） |
| `init` / `sync` / `project` が `--user` を受け付けない | `tests/commands/test_env_ops.py`（引数解析が非ゼロで終わること） |
