...
要求と受け入れ条件は `issues/PLAN56_secret-group-paths.md` に、決定の理由は
`issues/PLAN56_secret-group-paths-decisions.md` にある。この文書は「どう作るか」だけを扱う。
...
`issues/PLAN56_secret-group-paths-decisions.md` にある（決定 1〜13）。
