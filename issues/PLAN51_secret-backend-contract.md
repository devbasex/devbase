...
| コマンド | 扱い |
| --- | --- |
| `env list` / `get` / `set` / `delete` | 出力形式は変えない。持ち主を選ぶ `--user` を足す（下記）。`-p` の意味、既定の宛先、`list` の既定の表示範囲、`get` の探索順は変えない。`SecretStore` 越しに動くため backend を問わない |
| `env project` / `sync` | 変えない。扱う参照はチーム単位のままで、`--user` を受け付けない |
| `env edit` | 出力形式は変えない。持ち主を選ぶ `--user` を足す。`-p` の意味は変えない。**保存先を直接エディタへ渡すのは平文の backend だけ**にし、それ以外は一時ファイル経由で `load_bytes()` / `save_bytes()` を呼ぶ（下記） |
| `env init --reset` | 出力形式は変えない。扱う参照はチーム共通のままで、`--user` を受け付けない。退避はファイル backend では現行どおり保存先を複製し、サーバ backend では読み出した値を age 暗号化して控える（下記） |
...
持ち主を選ぶ `--user` を `env list` / `get` / `set` / `delete` / `edit` の 5 つへ足す。軸の
選び方は 1 つの規則で決まる。

**`-p` は適用範囲の軸を、`--user` は持ち主の軸を選ぶ。片方の指定がもう片方の軸を動かさない。**
書き込む 3 つ（`set` / `delete` / `edit`）は 1 つの参照だけを宛先にし、指定の無い軸は既定
（適用範囲は共通、持ち主はチーム単位）を採る。読む 2 つ（`list` / `get`）は指定の無い軸を
絞らず、その軸の両方を見る。適用範囲の軸で読み書きの既定が違うのは現行のままで、持ち主の軸を
同じ形にそろえた結果である。理由は[決定 14](PLAN51_secret-backend-decisions.md) にある。

**`-p` は名前を取らない真偽フラグである。** 対象のプロジェクトは実行時のディレクトリが
`$DEVBASE_ROOT/projects/<name>` 配下かどうかで決まる（`cli.py:293,301,310,314` と
`env.py` の `_target_env()` / `_project_env()`）。`--user` はこの決め方に触れない。`projects/`
配下でない場所での `-p` は現行どおり断る。

参照を選ぶ `--user` は値を取らない真偽フラグである。`devbase env backend use --user <id>` の
`--user` は個人単位の識別子を設定する別の引数で、そちらは値を取る。

`env set` / `delete` / `edit` の宛先は次のとおり。

| 指定 | 宛先の参照 |
| --- | --- |
| どちらも付けない | チーム共通（現行と同じ） |
| `-p` だけ | チームのプロジェクト（現行と同じ） |
| `--user` だけ | 個人共通 |
| `--user -p` | 個人のプロジェクト |

`env list` は適用範囲を `-g`（共通だけ）と `-p`（プロジェクトだけ）で絞る。どちらも真偽
フラグである。持ち主の軸も同じく、指定が無ければ絞らない。

| 指定 | 表示する参照 |
| --- | --- |
| 何も付けない | チーム共通 + 個人共通 + チームのプロジェクト + 個人のプロジェクト |
| `-g` だけ | チーム共通 + 個人共通 |
| `-p` だけ | チームのプロジェクト + 個人のプロジェクト |
| `--user` だけ | 個人共通 + 個人のプロジェクト |
| `-g --user` | 個人共通 |
| `-p --user` | 個人のプロジェクト |

**存在しない参照の節は出さない。** 個人単位の参照を持たない backend では個人単位の節が 1 つも
出ず、出力は現行と同じになる（`env.py:345` の既定は共通と現在のプロジェクトの両方を出す）。
チーム共通の節は現行どおり、変数が 0 件でも出す。

`env get` は `-p` を持たない。適用範囲の軸は常に絞らず、現行の `cmd_env_get()`（`env.py:410`）と
同じく共通を先に、次に現在のディレクトリのプロジェクトを探す。持ち主の軸は `--user` で絞る。

| 指定 | 探索順 |
| --- | --- |
| `--user` を付けない | 個人共通 → チーム共通 → 個人のプロジェクト → チームのプロジェクト |
| `--user` | 個人共通 → 個人のプロジェクト |

同じ適用範囲では個人単位を先に見る。[決定 12](PLAN51_secret-backend-decisions.md) の重ね順で
個人単位がチーム単位に勝つのと同じ向きにするためである。適用範囲の順は現行のまま共通が先で、
`devbase up` の重ね順（プロジェクトが共通に勝つ）とは向きが逆になる。同じキーが共通と
プロジェクトの両方にあると、`env get` が返す値とコンテナへ載る値が食い違う。これは現行の
`cmd_env_get()` が持つ性質そのままで、前提 3 のため変えない。

**既定をチーム単位に置く。** 既定を個人単位にすると、backend を設定していない端末で
`devbase env set` の書き先が変わり、既定の挙動を変えないという前提 3 を満たせない。個人単位へ
書くときは `--user` を明示する。

**`--user` を足さないコマンドがある。** `env init` / `sync` / `project` は `-p` を持たず、扱う
参照が役割と実行位置で決まっている（`init` / `sync` は共通、`project` は現在のディレクトリの
プロジェクト）。この 3 つと `export` / `import` はチーム単位の参照だけを相手にし、`--user` を
受け付けない。個人単位へ値を入れるのは `devbase env set --user` である。

`--user` を付けた実行は、backend が個人単位の参照を持たないとき（`age` / `plaintext` /
`auto`）、その backend が個人単位を持たない旨を述べて非ゼロ終了する。書き先を黙ってチーム単位へ
落とすと、個人の資格情報が全員から見える場所へ入る。読む側（`list` / `get`）で `--user` を
付けない実行は、個人単位の参照が無いだけとして扱い、現行どおり 0 で終わる。
