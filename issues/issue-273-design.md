...
| I2 | 参照の内容 | `env sync` が書くキーの宛先は「`--user` なら個人共通。無ければ、個人共通にあれば個人共通、無くチーム共通にあればチーム共通、どちらにも無ければ個人共通（個人単位の参照を持たないファイルの backend ではチーム共通）」の 1 つ（どちらにも無いときの既定は決定 12） | 宛先の判定を 1 か所（`SyncTargets.target_for`）に置き、ほかの場所で宛先を決めない |
...
- ホスト接続情報（`HOST_SSH_USER` / `HOST_SSH_HOST`）は `SyncTargets.get` で両方の参照を見て、どちらにも
  無いときだけ補う。宛先は I2（どちらにも無いため個人共通。ファイルの backend ではチーム共通。決定 12）
