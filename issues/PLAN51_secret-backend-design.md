...
ときだけで、失敗とは**通信できない場合（接続不能・タイムアウト・5xx）と応答が不正な場合
（認証拒否・JSON として読めない・`secrets` が配列でない）**を指す。
...
書き換えないのは取得に失敗したときだけである（前提 4）。失敗は通信できない場合と応答が不正な
場合に限り、機密が 0 件の正常な応答は成功として空の世代へ置き換える。
...
| 不達 + キャッシュなし → 非ゼロ終了 | 同上 |
| 通信失敗・不正な応答でキャッシュが変化しない | `tests/env/test_cache.py`（偽サーバを落とした場合と、500 や壊れた JSON を返す場合で、ファイルのハッシュが変わらないこと） |
