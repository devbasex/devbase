...
変えるのは注記と引用だけで、イメージの中の振る舞い（`base-image` と `tmux` のコンテキスト）は
変えない。
...
| base のシェルスクリプト | `containers/base/` の直下の各ファイル | 1 本のファイル | — | 指摘・抑止の注記 |
...
| I2 | base のシェルスクリプト | shellcheck 0.11.0 の既定の水準で指摘が 0 件 | CI の ShellCheck ジョブが失敗する |
| I3 | base のシェルスクリプト | shellcheck の指示を置いた行は、同じ行か直前の行に理由を持つ | pytest が失敗し、ファイル名と行番号を出す |
| I4 | base のシェルスクリプト | 注記・引用・変数名の変更の前後で、ファイルの振る舞いが変わらない | 既存の source するテストが失敗する |
...
| E2 | ShellCheck ジョブが `containers/base/` のシェルを検査した | ShellCheck ジョブ | Pull Request の検査の結果（指摘があれば失敗） |
...
| 検査の対象 | CI の ShellCheck ジョブが `shellcheck` の引数に並べたファイル | 追加（`ci`） |
| shellcheck の指示 | `# shellcheck` で始まるコメント（`disable=` / `source=` / `shell=`）。抑止の注記の指示の側 | 追加（`ci`） |
...
| F2 | 既存の 10 件の指摘を、1 件ずつ直すか抑止の注記にする | 同上 |
...
| 2 | `entrypoint.sh:631` | SC2046（`$(id -u)`） | 直す（決定 4） | `"$(id -u)"` | 変わらない（決定 4 の場合分け） |
| 3 | `entrypoint.sh:631` | SC2046（`$(id -g)`） | 直す（決定 4） | `"$(id -g)"` | 同上 |
| 4 | `entrypoint.sh:652` | SC2046（`$(id -u)`） | 直す（決定 4） | `"$(id -u)"` | 同上 |
| 5 | `entrypoint.sh:652` | SC2046（`$(id -g)`） | 直す（決定 4） | `"$(id -g)"` | 同上 |
...
表の判断は 3 種で、直す 5 件（2〜6）、抑止の注記 3 件（1・7・10）、指示を足す 2 件（8・9）である。

書き換えた後の形は次のとおりにする。理由の文面は実装で整えてよいが、**理由は指示の直前の行に置く**（決定 1）。

```bash
# entrypoint.sh の 1（#7 も同じ形）
...
試しに 10 件をこの形へ書き換えた写しを一時ディレクトリで検査し、shellcheck 0.11.0（base イメージ）と
...
3. 各行を `shlex.split(行, comments=True)` で語に分け、先頭の語の basename が `shellcheck` の行だけを採る
4. 採った行の、`-` で始まらない語を検査の対象とする。先頭の `./` は取り除く

漏れの検査は、数えた各ファイルの `containers/base/<名前>` が検査の対象に含まれることを確かめる。
...
- 上へ向かって続く指示の行を飛ばした最初の行が、shebang でない空でないコメント（`#` の後に 1 文字以上）である
...
### 決定 4: SC2046（`$(id -u)` / `$(id -g)` の 4 件）は引用を足して直す

`id -u` と `id -g` の出力は、成功すれば空白もグロブの文字も持たない 10 進の 1 語で、引用の有無で
引数は変わらない。失敗して出力が空のときは、引用が無いと引数が消えて `install -m 600 -o -g ...` になり
`-g` を持ち主の名前と読んで失敗し、引用があると `-o ""` で空の名前として失敗する。どちらも `install` が
非 0 を返して `||` の右（`cat` で書いて `chmod 600`）へ進み、行き先は同じである。**よって振る舞いの変わる
場合が無く、境界の「確認してから行う」には当たらない。**

この 2 か所は `DEVBASE_ENTRYPOINT_LIB_ONLY` の `return` より後にあり、source するテストでは通らない。
振る舞いが変わらないことは上の場合分けで示し、テストを足さない（「未確認のまま残ること」）。
...
base イメージの中身はコメントと引用と変数名だけが変わり、利用者から見える振る舞い・コマンド・出力は
...
| 条件 2: CI が 7 本を既定の水準で検査する | `test_base_shellcheck_ci.py` の漏れの検査が、7 本すべてを検査の対象に見つける。同じファイルに、検査の対象の `shellcheck` の行が `--severity` / `-S` を持たないことの検査を置く。実装 Pull Request のジョブのログに 7 本のパスが出る |
...
| 条件 5: 抑止の注記に理由がある / I3 | `test_base_shellcheck_ci.py` の指示の検査を 7 本へ走らせて失敗が 0 件。規則の単体テスト: 理由の無い指示・直前が空行の指示・直前が shebang の指示を失敗と数え、直前の行の理由・同じ行の ` # 理由`・指示が 2 行続いてその上に理由がある形を通す |
| 条件 6: 振る舞いが変わらない / I4 | `tests/containers/test_entrypoint_*.py`・`test_ai_cli_aliases.py`・`test_shellrc_dir.py` の 158 件（2026-09-26 に collect した数）がすべて通り、数が減らない。source する経路で通らない 5 件の書き換え（決定 4・5）は場合分けで示す |
...
| 項目 | 内容 |
| --- | --- |
| source する経路で通らない書き換え | `entrypoint.sh` の引用の追加（決定 4）と変数名の変更（決定 5）は、`DEVBASE_ENTRYPOINT_LIB_ONLY` の `return` より後にあり、テストで実行されない。場合分けで変わらないことを示す。base を建て直したコンテナでの確かめは対象範囲に含まない |
