...
| I2 | 名前の規則 | 13 か所の走査は、この述語を必ず通す前段の絞り込みとして使う。述語が偽を返す名前を数える走査は無い。S2・S9・S13 は述語の後に、走査ごとの名前の検査を残してよい（「走査（S1〜S13 に共通）」の節） | 述語を差し替えるテストが落ち、数えた走査を示す |
...
| E4 | 機密をプロジェクトの保存先へ書き込んだ | ファイルの backend の `save_bytes`・`env edit` の直接編集・`env import` の `_import_atomic` の書き込み | 保存先のファイル |
...
| 名前の形の説明 | 名前の形を利用者へ説明する文。`utils/names.NAME_FORM_HINT` 1 つから出す | 追加（`cli`） |

「名前の形」「参照」「backend」は用語集にある意味のまま使う。表の 3 つの語の正本は、今は
