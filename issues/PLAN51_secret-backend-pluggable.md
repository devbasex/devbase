...
- [ ] `--user` は持ち主の軸だけを切り替える。`devbase env set KEY=VALUE --user -p` の宛先は
      個人のプロジェクトで、`-p` だけを付けた実行の宛先はチームのプロジェクトのまま変わらない。
      `-p` は現行どおり名前を取らない真偽フラグで、対象のプロジェクトは実行時のディレクトリから
      決まり、`projects/` 配下でなければ現行と同じ理由で断られる
- [ ] backend が `age` / `plaintext` / 未設定のとき、`--user` を付けない `devbase env list` の
      出力は現行と同じ（共通と現在のプロジェクトの両方を出す）で、`devbase env get` は現行と
      同じく共通に無ければ現在のプロジェクトを探す。`tests/commands/test_env_ops.py` と
      `tests/commands/test_env_store_switch.py` を書き換えずに通る
- [ ] `devbase env init` / `sync` / `project` は `--user` を受け付けず、扱う参照はチーム単位の
      ままになる
...
| 対象 | 影響 |
| --- | --- |
| 公開インタフェース | **増える**。`devbase env backend` 系のサブコマンドと、参照の持ち主を選ぶ `--user` を `env list` / `get` / `set` / `delete` / `edit` へ追加する。`-p` の意味、既定の宛先、`list` の既定の表示範囲、`get` の探索順、出力形式は変えない |
