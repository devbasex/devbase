...
| キャッシュ | サーバから取得した機密を、不達時に備えて手元へ age 暗号化して控えたもの。認証を拒まれたときは使わない |
...
- 前提 4: キャッシュは**サーバから取得できたときだけ**更新する。ここでの「取得できた」は
  ...
  失敗したとき — 通信できない、応答を解釈できない、または認証を拒まれたとき — だけである。
  残したキャッシュを使ってよいかは前提 6 が別に決める。（成否の判定: 通信失敗・解釈できない
  応答・認証拒否の後はキャッシュファイルの中身が変わらず、機密 0 件の正常な応答の後は
  キャッシュがその結果で置き換わること）
