...
- 前提 4: キャッシュは**サーバから取得できたときだけ**更新する。取得に失敗したときは古い
  キャッシュを消さない。（成否の判定: 取得失敗後もキャッシュファイルの中身が変わらないこと）
...
- `SecretBackend` を**明示的に選べる登録簿**へ広げる（`SecretStore` の生成を 1 か所の factory へ集約する）
...
- [ ] `devbase env backend migrate --to infisical` は、サーバへ書き込んだ後に**読み戻して元と
      一致することを確認**し、一致しなければ書き込んだ分を消して非ゼロで終了する
...
- [ ] サーバからの取得に失敗したとき、既存のキャッシュファイルの内容が変化しない
...
| 既存の振る舞い | backend を設定するまで変わらない。`SecretStore` の生成が factory 経由へ変わるが、既定の解決結果は現状と同じ |
...
| 区分 | 内容 |
| --- | --- |
| 常に行う | 既存テストの実行、`SecretStore` 生成箇所の factory への置き換え、値をログへ出さないことの確認 |
