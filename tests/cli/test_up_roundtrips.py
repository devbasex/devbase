...
# ---------------------------------------------------------------------------
# R3-004 現状固定: `_ensure_env_files` の異常系 (extract_method の前に固定)
# ---------------------------------------------------------------------------

def _pop_global_and_project(openbao):
    """`web` の共通機密とプロジェクト機密を落とし、env init と project .env 作成を通す。"""
    for path in (TEAM_GLOBAL, TEAM_WEB):
        openbao.secrets.pop(path, None)
...
def test_ensure_env_files_nonzero_exit_returns_false_but_creates_project_env(
    ...
    さらに終了コードによらずストアのキャッシュは解放され、次の `runtime.resolve` は
    サーバから読み直す。
    """
    root = up_root['root']
    ...
    monkeypatch.setattr(container.subprocess, 'run', fake_run)
    # 事前にストアを載せておき、解放されたことをログイン回数の増分で見る
    runtime.resolve(root)
    logins_before = openbao.logins

    assert container._ensure_env_files() is False
    # 子プロセスが失敗しても project .env の作成は試みる
    assert project_env.exists()
    # 失敗後もキャッシュは解放され、resolve が新たに認証する
    runtime.resolve(root)
    assert openbao.logins > logins_before


def test_ensure_env_files_env_init_launch_exception_returns_false(
        up_root, openbao, monkeypatch):
    ...
    assert container._ensure_env_files() is False
    # 例外でも後続の project .env 作成は続く
    assert project_env.exists()


def test_ensure_env_files_project_env_touch_failure_returns_false(
        up_root, openbao, monkeypatch):
    ...
    monkeypatch.setattr(container.subprocess, 'run', ok_run)

    real_touch = Path.touch

    def failing_touch(self, *a, **k):
        if self.name == '.env':
